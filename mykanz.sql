...
-- Budget tanpa category_id (digantikan oleh tabel budget_categories)
...
-- Relasi budget ke banyak kategori (Multi-Kategori)
...
-- Goals dengan dukungan tracking portofolio aset (di sini karena FK ke assets)
CREATE TABLE goals (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    ...
    target_asset_units DECIMAL(18, 8),
    current_asset_units DECIMAL(18, 8) DEFAULT 0,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Aktifkan RLS hanya pada tabel users (tabel lain di-bypass untuk custom auth POC)
ALTER TABLE public.users ENABLE ROW LEVEL SECURITY;

-- Policy users: open access untuk support custom auth tanpa Edge Function / JWT
...
-- Semua tabel lain: RLS dinonaktifkan (custom auth POC tanpa JWT Supabase)
...
-- Grant akses read views ke role authenticated
